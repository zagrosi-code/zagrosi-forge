...
5. Run setup for a mutable legacy plan:

```bash
python3 {plugin_root}/scripts/zagrosi_skills.py implement-setup --sections-dir "{sections_dir}" --target-dir "{target_dir}"
```

For an admitted plan whose bytes must remain immutable, use detached frozen-
planning mode. `{implementation_root}` must be disjoint from the planning tree,
and `{admission_pinner}` must be a pre-existing external 0600 canonical JSON
...
Before every section, run `next-section` with the same optional
`--implementation-root`. Implement only a member of its `ready_sections`; use
`next_section` as the deterministic default. Raw `SECTION_MANIFEST` order is not
implementation authority when the dependency graph is non-topological. Both
legacy and detached recording reject a section absent from the manifest or any
section whose declared predecessors are unknown or not yet recorded complete.
...
The packet command above is legacy mode only. In detached frozen-planning mode,
either skip packet generation or require an explicit output beneath the
external implementation root:

```bash
python3 {plugin_root}/scripts/zagrosi_skills.py implementation-packet \
  --planning-dir "{planning_dir}" --section "{section}" \
  --implementation-root "{implementation_root}" \
  --output-dir "{implementation_root}/code_review/generated/packets"
```

Never use its default output in detached mode; the tool refuses a missing or
planning-local output directory.
...
That command is also legacy-only as written. In detached mode, skip it or use:

```bash
python3 {plugin_root}/scripts/zagrosi_skills.py tdd-skeletons \
  --planning-dir "{planning_dir}" --framework pytest \
  --implementation-root "{implementation_root}" \
  --output-dir "{implementation_root}/code_review/generated/tdd-skeletons"
```
...
```bash
python3 {plugin_root}/scripts/zagrosi_skills.py implement-progress \
  --planning-dir "{planning_dir}" \
  [--implementation-root "{implementation_root}"] \
  --section "{section}" --stage red \
  --result "targeted test failed as expected"
```

### 4. Stage and Review

If this is a git repo, stage only files related to the section. In legacy mode,
save the diff to:
...
In detached frozen-planning mode, save it to
`{implementation_root}/code_review/{section}-diff.md` instead. Never write a
diff, review or decision beneath the admitted planning root.
...
Write the review in the same mode-specific review directory:

```text
{planning_dir}/implementation/code_review/{section}-review.md
{implementation_root}/code_review/{section}-review.md  # detached only
```
...
Record decisions in the same mode-specific review directory:

```text
{planning_dir}/implementation/code_review/{section}-decisions.md
{implementation_root}/code_review/{section}-decisions.md  # detached only
```

### 6. Update Section Documentation

Before committing in legacy mode, update the section file so it reflects what
was actually built. Use `references/section-update.md`. In detached frozen-
planning mode, section documentation is immutable: record implementation facts
only in external review, decision, evidence and pinner records.
...
Record completion in legacy mode as before:
...
In detached frozen-planning mode, add the same `--implementation-root`, use
root-relative `code_review/...` artifacts, and bind any canonical external
result with `--evidence-row`:
...
Legacy mode updates implementation state and refreshes
`{planning_dir}/traceability.md`. Detached mode writes canonical state and a
content-addressed section pinner externally, refuses unknown or incomplete
predecessors, and never refreshes plan-local traceability or section files.
...
verification evidence. Legacy mode then refreshes traceability. In detached
mode a leaf section may create a new content-addressed pinner in place, but a
section with any completed dependant cannot be re-recorded. Start a new detached
root/state and replay the dependency order when final hashes require replacing
an already-pinned predecessor; never leave completed dependants pointing at an
old predecessor receipt.
...
2. Write `{planning_dir}/implementation/usage.md` with practical usage notes in
   legacy mode, or `{implementation_root}/code_review/usage.md` in detached
   mode; the detached top-level allowlist forbids a root-level usage file.
3. If using a consolidated commit, stage the completed implementation once and
   commit it in the repo's normal style.
4. In legacy mode, run the existing final gates:
...
```
   ...
   In detached mode, rerun `next-section --planning-dir "{planning_dir}"
   --implementation-root "{implementation_root}"` and require empty
   `remaining_sections`, a null `next_section`, unchanged planning/admission
   hashes, and every completed pinner to reopen successfully. Do not run a
   report or traceability writer against the frozen planning root.
