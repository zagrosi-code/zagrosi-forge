...
from test_zagrosi_skills import load_zagrosi_module, write_lean_plan_fixture
...
def call_gate(forge, monkeypatch, capsys, tmp_path, command, **kwargs):
    def status(_args):
        return forge.print_json(forge.run_internal_gate(command[0], command, **kwargs))

    monkeypatch.setattr(forge.owner("status"), "status", status)
    assert forge.main(["status", "--path", str(tmp_path)]) == 0
...
def test_local_plan_flights_do_not_start_python_children(forge, monkeypatch, capsys, plan, phase):
    ...
    monkeypatch.setattr(forge.subprocess, "run", forbidden)
...
def test_local_gate_matches_process_payload(forge, monkeypatch, capsys, tmp_path, plan, strict_failure):
    ...
    expected = forge.run_internal_gate("lint-plan", command)
...
def test_pretty_flight_and_following_json_are_isolated(forge, capsys, plan):
    assert forge.main(["postflight", "--phase", "plan", "--planning-dir", str(plan), "--pretty"]) == 0
    assert not capsys.readouterr().out.startswith("{")
    assert forge.main(["status", "--path", str(plan)]) == 0
    output = capsys.readouterr().out
    assert len(output.splitlines()) == 1
    assert json.loads(output)["success"]
    assert forge._CLI_CONTEXT.get() is None
    assert forge._GATE_STREAMS.get() is None
...
def test_local_gate_failures_restore_streams_cwd_and_timer(forge, monkeypatch, capsys, tmp_path, failure):
    ...
    def doctor(_args):
        if failure == "no-output":
            return 0
        forge.print_json({"success": True})
        if failure == "exit-after-output":
            raise SystemExit(3)
        raise RuntimeError("test command failed")

    monkeypatch.setattr(forge.owner("doctor"), "doctor", doctor)
...
def test_local_gate_timeout_retains_failure_contract(forge, monkeypatch, capsys, tmp_path):
    ...
    def doctor(_args):
        time.sleep(1)
        return forge.print_json({"success": True})

    monkeypatch.setattr(forge.signal, "setitimer", short_timer)
    monkeypatch.setattr(forge.owner("doctor"), "doctor", doctor)
...
def test_nonmain_invocation_uses_processes_without_changing_signal_handlers(forge, monkeypatch, capsys, plan):
    ...
    monkeypatch.setattr(forge.subprocess, "run", process)
    monkeypatch.setattr(forge.signal, "setitimer", forbidden, raising=False)
    with ThreadPoolExecutor(max_workers=1) as executor:
        result = executor.submit(forge.main, ["postflight", "--phase", "plan", "--planning-dir", str(plan)]).result()
    assert result == 0
    assert calls
...
def test_platform_without_interval_timer_uses_process_fallback(forge, monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.delattr(forge.signal, "setitimer", raising=False)
    monkeypatch.setattr(forge.subprocess, "run", process)
...
def test_local_postflight_builds_one_parser_and_reads_each_file_once(forge, monkeypatch, capsys, plan):
    ...
    original_read, original_parser = Path.read_text, forge.build_parser
    ...
    monkeypatch.setattr(forge.owner("build_parser"), "build_parser", parser)
    assert forge.main(["postflight", "--phase", "plan", "--planning-dir", str(plan)]) == 0
    assert json.loads(capsys.readouterr().out)["success"]
...
def test_read_cache_detects_same_length_rewrite_and_does_not_cross_invocations(forge, monkeypatch, capsys, tmp_path):
    ...
    def status(_args):
        assert forge.read_text(path) == forge.read_text(path) == "old"
        old_stat = path.stat()
        path.write_text("new")
        os.utime(path, ns=(old_stat.st_atime_ns, old_stat.st_mtime_ns))
        assert forge.read_text(path) == "new"
        return forge.print_json({"success": True})
...
def test_detached_handoff_retains_exact_shape_and_bypasses_runtime_cache(forge, monkeypatch, capsys, tmp_path):
    section = next(iter(forge.HANDOFF_SECTION_CONTRACTS))

    def handoff(_args):
        assert forge._CLI_CONTEXT.get()["texts"] is None
        assert not forge._CLI_CONTEXT.get()["local_gates"]
        return forge.print_json({"success": True})

    monkeypatch.setattr(forge.owner("detached_implement_evidence_handoff"), "detached_implement_evidence_handoff", handoff)
    command = ["implement-evidence-handoff", "--implementation-root", str(tmp_path), "--section", section]
    assert forge.main([*command, "--pretty"]) == 2
