...
@dataclass
class _Recovery:
    """Own the journal descriptor until commit or cleanup consumes it."""
    ...
    transaction_fd: int
    close_transaction_fd: bool = True

    def validate_rollback_base(self) -> None:
        self.require_lock_authority()
        _detached_authority.verify_detached_authorities(
            self.planning_dir, self.implementation_root, self.root_fd, self.config, self.guard,
        )
        _pinners.detached_completed_records(self.root_fd, self.config, self.progress)
        self.require_lock_authority()

    def rollback(self, inventory: set[str]) -> None:
        ...
        transaction, transaction_raw = _secure_io.load_canonical_json_at(self.transaction_fd, "rollback.json")
        state, current_state_raw = _secure_io.load_canonical_json_at(self.root_fd, "zagrosi_implement_state.json")
        ...
        if not _transaction_io.execute_section_record_rollback(
            ...
            base_state,
            base_raw,
            self.validate_rollback_base,
        ):
            self.close_transaction_fd = False
            raise _models.DetachedImplementationError(
                "section-record-recovery-required",
    ...
    def discard_unpublished(self, inventory: set[str]) -> None:
        if "state.json" in inventory:
            raise _models.DetachedImplementationError(
                "section-record-recovery-required",
        ...
        if "pinner.tmp" in inventory:
            ...
            transaction, _ = _secure_io.load_canonical_json_at(self.transaction_fd, "transaction.tmp")
            ...
            pinners_fd = _secure_io.open_relative_directory(self.root_fd, "pinners")
            try:
            ...
            _pinners.detached_completed_records(self.root_fd, self.config, self.progress)
        self.require_lock_authority()
        _detached_authority.verify_detached_authorities(self.planning_dir, self.implementation_root, self.root_fd, self.config, self.guard)
        if not _transaction_io.cleanup_section_record_transaction_after_commit(self.root_fd, self.transaction_fd):
            self.close_transaction_fd = False
            raise _models.DetachedImplementationError(
    ...
    def finish(self, inventory: set[str]) -> None:
        ...
        transaction, transaction_raw = _secure_io.load_canonical_json_at(self.transaction_fd, "transaction.json")
        state, current_state_raw = _secure_io.load_canonical_json_at(self.root_fd, "zagrosi_implement_state.json")
        _handoff_wire.require_exact_fields(state, _detached_contract.DETACHED_STATE_FIELDS, "Detached implementation state")
        ...
        def rollback_candidate_after_recovery_failure(cause: Exception) -> None:
            _, observed_raw = _secure_io.load_canonical_json_at(self.root_fd, "zagrosi_implement_state.json")
        ...
        if not final_present:
            ...
            _transaction_io.install_staged_section_pinner(self.root_fd, self.transaction_fd, pinner_path, staged_raw)
        _transaction_io.section_record_pinner_relation(self.root_fd, self.transaction_fd, pinner_path, staged_raw)
        ...
        if current_state_raw == base_raw:
            try:
                ...
                _transaction_io.replace_state_from_transaction(self.root_fd, self.transaction_fd, base_raw, candidate_state)
        ...
        try:
            ...
            rollback_candidate_after_recovery_failure(exc)
        if not _transaction_io.commit_section_record_transaction(self.root_fd, self.transaction_fd):
            self.close_transaction_fd = False
            raise _models.DetachedImplementationError(
                "section-record-committed-cleanup-pending",
...
def recover_section_record_transaction_locked(
    ...
    if transaction_fd is None:
        return
    recovery = _Recovery(planning_dir, implementation_root, root_fd, config, guard,
                         progress, require_lock_authority, transaction_fd)
    try:
        ...
        if "rollback.json" in inventory:
            recovery.rollback(inventory)
        elif "transaction.json" not in inventory:
            recovery.discard_unpublished(inventory)
        else:
            recovery.finish(inventory)
    finally:
        if recovery.close_transaction_fd:
